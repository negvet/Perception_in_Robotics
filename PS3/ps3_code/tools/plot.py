...
from tools.task import get_gaussian_statistics_xy
...
def plot2dcov(mean, covariance, k, number_of_points):
    ...
    # ax.set_aspect('equal')
    plt.plot(final_points.T[0], final_points.T[1])



def plot_field(field_map, detected_landmarks, lm_positions, lm_correspondences):
    """
    ...
    for k in range(field_map.num_landmarks):
        ...
        if k in detected_landmarks:
            ...
            # print('landmark', k, 'detected')
            # print('lm_correspondences',lm_correspondences)
            ...
            # determine new lm id
            new_lm_id = None
            for corresp in lm_correspondences:
                if k == corresp[0]:
                    new_lm_id = corresp[1]

            if new_lm_id is not None:

                lm = lm_positions[new_lm_id]

                # for idx,lm in enumerate(lm_positions):
                if len(lm)>5:
                    lm_mu, lm_sigma = get_gaussian_statistics_xy(np.array(lm[-5:]))
                    plot2dcov(lm_mu, lm_sigma, 3, 50)
