...
from tools.plot import plot_pose_errors
...
def main():
    ...
    with movie_writer.saving(fig, args.movie_file, data.num_steps) if write_movie else get_dummy_context_mgr():
        for t in range(data.num_steps):
            ...
            if update_mean_trajectory:
                if one_trajectory_per_particle:
                    sim_trajectory.mean[t, :, :] = localization_filter.particle_set.T
                else:
            ...
            if show_particles:
                samples = localization_filter.particle_set.T
    ...
    if show_plots:
        plt.show(block=True)
        plot_pose_errors(data, sim_trajectory, args)
