...
def sample_from_odometry(state, motion, alphas):
    ...
    # sample1d ~ output a single sample (one number) from normal distribution N(mean,standard_deviatiopn)
    ...
    # Ex: if posision.x_prev == 10, motion = 1, noisy_motion = 1.12, position.x = 11.12

    return get_prediction(state, noisy_motion) # let's see where do we get ith this noisy motion (in accordance with our transformation(motion) function)
...
def get_jacobian_G(state, motion):
    """
    :param state: The current state of the robot (format: [x, y, theta]).
    :param motion: The motion command to execute (format: [drot1, dtran, drot2]).
...
def get_jacobian_V(state, motion):
    """
    :param state: The current state of the robot (format: [x, y, theta]).
    :param motion: The motion command to execute (format: [drot1, dtran, drot2]).
