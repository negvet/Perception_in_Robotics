...
def plot_pose_errors(data, sim_trajectory, args):
    fig, ax = plt.subplots(3, 1, figsize=(12, 20))

    if args.filter_name == 'ekf':
        fig.suptitle('EKF', fontsize=25)
        fig_title = 'ekf.png'
    else:
        fig.suptitle('PF', fontsize=25)
        fig_title = 'pf.png'

    ax[0].plot(np.abs(data.debug.real_robot_path[:, 0] - sim_trajectory.mean[:, 0]), label=r"$abs(x_f - x_t)$")
    ax[0].plot(3 * sim_trajectory.covariance[0, 0, :] ** (0.5), label=r"$3\sigma$")
    ax[0].legend()
    ax[0].set_title('x')

    ax[1].plot(np.abs(data.debug.real_robot_path[:, 1] - sim_trajectory.mean[:, 1]), label=r"$abs(y_f - y_t)$")
    ax[1].plot(3 * sim_trajectory.covariance[1, 1, :] ** (0.5), label=r"$3\sigma$")
    ax[1].legend()
    ax[1].set_title('y')

    ax[2].plot(np.abs((data.debug.real_robot_path[:, 2] - sim_trajectory.mean[:, 2] + np.pi) % (2 * np.pi) - np.pi), label=r"$abs(\theta_f - \theta_t)$")
    ax[2].plot(3 * sim_trajectory.covariance[2, 2, :] ** (0.5), label=r"$3\sigma$")
